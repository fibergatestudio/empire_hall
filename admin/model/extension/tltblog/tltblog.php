...
class ModelExtensionTltBlogTltBlog extends Model {
	public function addTltBlog($data) {
		$this->db->query("INSERT INTO " . DB_PREFIX . "tltblog SET sort_order = '" . (int)$data['sort_order'] . "', image = '" . (isset($data['image']) ? $this->db->escape($data['image']) : '') . "', bottom = '" . (isset($data['bottom']) ? (int)$data['bottom'] : 0) . "', show_title = '" . (isset($data['show_title']) ? (int)$data['show_title'] : 0) . "', date_available = '" . $data['date_available'] . "', status = '" . (int)$data['status'] . "', show_description = '" . (isset($data['show_description']) ? (int)$data['show_description'] : 0) . "'");
        ...
        if (isset($data['cover_image'])) {
			foreach ($data['cover_image'] as $image) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "tltblog_image SET tltblog_id = '" . (int)$tltblog_id . "', title = '" . $this->db->escape($image['title']) . "', image = '" . $this->db->escape($image['image']) . "', sort_order = '" . (int)$image['sort_order'] . "'");
				$tltblog_image_id = $this->db->getLastId();
				foreach ($image['title'] as $key => $title) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "tltblog_image_description SET "
							. "tltblog_image_id = '" . (int)$tltblog_image_id . "', "
							. "tltblog_id = '" . (int)$tltblog_id . "', "
							. "language_id = '" . (int)$key . "', "
	...
	public function editTltBlog($tltblog_id, $data) {
		...
		$this->db->query("DELETE FROM " . DB_PREFIX . "tltblog_image WHERE tltblog_id = '" . (int)$tltblog_id . "'");
		$this->db->query("DELETE FROM " . DB_PREFIX . "tltblog_image_description WHERE tltblog_id = '" . (int)$tltblog_id . "'");
        if (isset($data['cover_image'])) {
			foreach ($data['cover_image'] as $image) {
				$this->db->query("INSERT INTO " . DB_PREFIX . "tltblog_image SET tltblog_id = '" . (int)$tltblog_id . "', image = '" . $this->db->escape($image['image']) . "', sort_order = '" . (int)$image['sort_order'] . "'");
				$tltblog_image_id = $this->db->getLastId();
				foreach ($image['title'] as $key => $title) {
					$this->db->query("INSERT INTO " . DB_PREFIX . "tltblog_image_description SET "
							. "tltblog_image_id = '" . (int)$tltblog_image_id . "', "
	...
	public function getTltBlogsGallery($tltblog_id) {

		$mass_result = array();
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "tltblog_image WHERE tltblog_id = '" . (int)$tltblog_id . "' ORDER BY sort_order ASC");

			foreach ($query->rows as $value) {
				$mass_title = array();
				$query_title = $this->db->query("SELECT * FROM " . DB_PREFIX . "tltblog_image_description WHERE tltblog_image_id = '" . (int)$value['tltblog_image_id'] . "'");
				foreach ($query_title->rows as $value_title) {
					$mass_title[$value_title['language_id']] = $value_title['title'];

				}
				$mass_result[] = array(
					'tltblog_image_id' => $value['tltblog_image_id'],
					'tltblog_id' => $value['tltblog_id'],
					'image' => $value['image'],
