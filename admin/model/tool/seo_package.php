...
class ModelToolSeoPackage extends Model {
  ...
  public function getManufacturerData($manufacturer_id, $lang, $store_id) {
      ...
      //custom code
      $query = $this->db->query("SELECT md.name as orig_name, d.name, d.description as 'desc', d.meta_description as meta_desc, d.meta_title as title, d.meta_keyword as keywords, d.seo_h1 as h1, d.seo_h2 as h2, d.seo_h3 as h3 FROM " . DB_PREFIX . "manufacturer m LEFT JOIN " . DB_PREFIX . "seo_manufacturer_description d ON (m.manufacturer_id = d.manufacturer_id AND d.language_id=" . (int) $lang . " AND d.store_id = '" . (int) $store_id . "') LEFT JOIN " . DB_PREFIX . "manufacturer_description md ON (m.manufacturer_id = md.manufacturer_id AND d.language_id=" . (int) $lang . ") WHERE m.manufacturer_id = '" . (int)$manufacturer_id . "' LIMIT 1")->row;
