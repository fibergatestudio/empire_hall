...
class ModelToolExchange1c extends Model {
	...
	public function queryOrders($params) {
		...
		$objNow = new SimpleXMLElement($root);
		$xml = $this->array_to_xml($document, $objNow);
	...
	public function parseImport($filename, $language_id) {
		...
		if ($xml->Каталог->Товары->Товар) {
			foreach ($xml->Каталог->Товары->Товар as $product) {
				...
				$data['1c_id'] = $uuid[0];
                $data['upc'] = $product->Штрихкод? (string)$product->Штрихкод : '';
				$data['model'] = $product->Артикул? (string)$product->Артикул : 'не задана';
                $data['sku'] = $product->Код? (string)$product->Код : '';
	...
	*/
	...
	private function initProduct($product, $data = array(), $language_id) {
		...
		$result['product_description'] = array(
			...
			$language_id => array(
				'name'              => isset($product['name']) ? $product['name'] : (isset($data['product_description'][$language_id]['name']) ? htmlspecialchars($data['product_description'][$language_id]['name'], ENT_QUOTES): 'Имя не задано')
	...
	private function setProduct($product, $language_id) {
		...
		if ($product_id) {
			...
			if ($this->config->get('exchange1c_dont_use_artsync')) {
				$this->load->model('catalog/product');
				$product_id =	$this->model_catalog_product->addProduct1c($data);
			} else {
				...
				// Если нет, то создаем новый
					...
					$this->load->model('catalog/product');
					$this->model_catalog_product->addProduct1c($data);
					$product_id = $this->getProductBySKU($data['sku']);
				}
			}
	...
	private function updateProduct($product, $product_id = false, $language_id) {
		...
		//Редактируем продукт
		$product_id = $this->model_catalog_product->editProduct1c($product_id, $product_old);
