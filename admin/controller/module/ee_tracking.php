...
class ControllerModuleEeTracking extends Controller {
    private $error      = array();
    private $mName      = 'ee_tracking';
    private $mId        = '25965';
    private $mModel     = 'model_module_ee_tracking';
    private $mPath      = 'module/ee_tracking';
    private $mMainPath  = 'extension/module';
    ...
    protected function updateJsFile() {
        ...
        foreach ($stores as $store) {
            $checkout_custom = $this->{$this->mModel}->getSettingValue($this->mName . '_checkout_custom', $store['store_id']);

            if ($store['store_id'] != $store_id && $checkout_custom) {

                $store_checkout_url = json_decode($this->{$this->mModel}->getSettingValue($this->mName . '_checkout_url', $store['store_id']), $this->mSSL);
            ...
            $store_js_version = $this->{$this->mModel}->getSettingValue($this->mName . '_js_version', $store['store_id']);
