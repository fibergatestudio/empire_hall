...
class ControllerCustomerCustomer extends Controller {
    ...
    public function export() {
        ...
        require_once(DIR_SYSTEM . 'PHPExcel/PHPExcel/Writer/Excel5.php');

        $this->load->model('customer/customer');

        $registers = $this->model_customer_customer->getCustomers();
        ...
        if($registers) {
            ...
            $worksheet1->setTitle( 'Export Клиентов' );


            $Line = 2;

            $worksheet1->setCellValue('A1', 'ID');
            $worksheet1->setCellValue('B1', 'ФИО');
            $worksheet1->setCellValue('C1', 'Email');
            $worksheet1->setCellValue('D1', 'Телефон');
            $worksheet1->setCellValue('E1', 'День рождения');
            $worksheet1->setCellValue('F1', 'Дата регистрации');
            foreach($registers as $result) {
                $worksheet1->setCellValue('A'.$Line, $result['customer_id']);
                $worksheet1->setCellValue('B'.$Line, html_entity_decode($result['name']));
                $worksheet1->setCellValue('C'.$Line, $result['email']);
                $worksheet1->setCellValue('D'.$Line, $result['telephone']);
                $worksheet1->setCellValue('E'.$Line, $result['birthday']);
                $worksheet1->setCellValue('F'.$Line, date('d.m.Y', strtotime($result['date_added'])));
            ...
            $datetime = date('d-m-Y');
            $filename = 'customer-'. $datetime. '.xls';
