...
class ControllerCatalogProduct extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if(isset($this->request->post['kfc_feeld']) && $this->request->post['kfc_feeld']){
				$kcf_type = 2; //значит категории
				$this->model_extension_kcf->getKcfUpdateFeeld($kcf_type, $product_id, $this->request->post['kfc_feeld']);
			}
	...
	public function edit() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if(isset($this->request->post['kfc_feeld']) && $this->request->post['kfc_feeld']){
				$kcf_type = 2; //значит категории
				$this->model_extension_kcf->getKcfUpdateFeeld($kcf_type, $this->request->get['product_id'], $this->request->post['kfc_feeld']);
			}
			/* ------ END KCF ------ END KCF ------ END KCF ------ END KCF ------ END KCF ------ END KCF ------ */
			/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */				
	...
	protected function getForm() {
		...
		// мультидобавление Рекламных тегов
		$this->load->model('catalog/promotags');

		$data['promotags'] = $this->model_catalog_promotags->getPromoTags();

		if (isset($this->request->post['product_promo'])) {
			$product_promotags = $this->request->post['product_promo'];
		} elseif (isset($this->request->get['product_id'])) {
			$product_promotags = $this->model_catalog_product->getProductPromo($this->request->get['product_id']);
		} else {
			$product_promotags = array();
		}

		$data['product_promotags'] = array();

		foreach ($product_promotags as $product_promotag) {
			$data['product_promotags'][] = array(
				'promo_tags_id'     => $product_promotag['promo_tags_id'],
				'date_start'        => ($product_promotag['date_start'] != '0000-00-00') ? $product_promotag['date_start'] : '',
				'date_end'          => ($product_promotag['date_end'] != '0000-00-00') ? $product_promotag['date_end'] :  ''
			);
		}
		// end мультидобавление Рекламных тегов
		...
		$data['product_id'] = isset($this->request->get['product_id']) ? $this->request->get['product_id'] : $this->model_catalog_product->getNewProductId();
		
		/* получим сам набор полей, привязанный к странице */
		$data['kcf_feelds'] = $this->model_extension_kcf->getKcfFeelds($kcf_type, $data['product_id']);
		$data['img_start'] = HTTP_CATALOG . 'image/';
		
		/* Получим данные для Выбора типа полей */
		$data['kcftypes_now'] = $this->model_extension_kcf->getKcfTypeNow($kcf_type, $data['product_id']);
		/* ------ END KCF ------ END KCF ------ END KCF ------ END KCF ------ END KCF ------ END KCF ------ */
		/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */		
