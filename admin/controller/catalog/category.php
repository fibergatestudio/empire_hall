...
class ControllerCatalogCategory extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if(isset($this->request->post['kfc_feeld']) && $this->request->post['kfc_feeld']){
				$kcf_type = 1; //значит категории
				$this->model_extension_kcf->getKcfUpdateFeeld($kcf_type, $this->request->get['category_id'], $this->request->post['kfc_feeld']);
			}
	...
	protected function getForm() {
		...
		//если на странице создания, то id еще нет - возьмем следующий.
		$data['category_id'] = isset($this->request->get['category_id']) ? $this->request->get['category_id'] : $this->model_catalog_category->getNewCategoryId();
		
		/* получим сам набор полей, привязанный к странице */
		$data['kcf_feelds'] = $this->model_extension_kcf->getKcfFeelds($kcf_type, $data['category_id']);
		$data['img_start'] = HTTP_CATALOG . 'image/';
		
		/* Получим данные для Выбора типа полей */
		$data['kcftypes_now'] = $this->model_extension_kcf->getKcfTypeNow($kcf_type, $data['category_id']);
		/* ------ END KCF ------ END KCF ------ END KCF ------ END KCF ------ END KCF ------ END KCF ------ */
		/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */
