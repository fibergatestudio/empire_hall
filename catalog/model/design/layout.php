...
class ModelDesignLayout extends Model {
	...
	public function getGeneralLayoutTemplate($layout_id){
		$query = $this->db->query("SELECT * FROM " . DB_PREFIX . "layout_route WHERE layout_id = '" . $this->db->escape($layout_id) . "' AND store_id = '" . (int)$this->config->get('config_store_id') . "' AND template IS NOT NULL ORDER BY route DESC LIMIT 1");
		if ($query->num_rows) {
			return $query->row['template'];
		} else {
			return 0;
