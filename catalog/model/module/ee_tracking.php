...
class ModelModuleEeTracking extends Model {
    ...
    public function listView($data) {
        ...
        if ($client_id && $data) {
            $this->load->language('module/ee_tracking');
    ...
    public function refund($data) {
        ...
        if (isset($data['order_id'])) {
            ...
            if ($client_id) {
                $fields['dl'] = html_entity_decode($this->url->link('account/return/add', '', 'SSL'), ENT_QUOTES, 'UTF-8');
                $fields['cid'] = $client_id;
                $fields['ea'] = 'Refund';
    ...
    protected function setCurlRequest($fields, $debug = 0) {
        ...
        if (phpversion() < '5.4') {
            $fields_string = utf8_encode(str_replace('+', '%20', http_build_query($fields)));
        } else {
    ...
    protected function getCategoryProductPosition($product_id, $category_id, $query) {
        ...
        } else if (substr(VERSION, 0, 7) > '2.1.0.2') {
    ...
    public function getBannerImages($banner_id, $language_id) {
        ...
        AND bid.language_id = '" . (int)$language_id . "' 
