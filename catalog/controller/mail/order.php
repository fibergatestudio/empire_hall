...
class ControllerMailOrder extends Controller {
	...
	public function add($order_info, $order_status_id, $comment, $notify) {
		...
		$data['text_order_name'] = $language->get('text_order_name');
		$data['text_footer'] = $language->get('text_footer');

        $data['domain_name'] = DOMAIN_NAME;
        $data['domain_url'] = DOMAIN_URL;
        $data['order_copy'] = sprintf($language->get('text_order_copy'), date('Y'));
		...
		foreach ($order_products as $order_product) {
            ...
            $product_image = $this->db->query("SELECT image FROM " . DB_PREFIX . "product WHERE product_id = '" . $order_product['product_id'] . "'");

            if (is_file(DIR_IMAGE . $product_image->row['image'])) {
                $image = $this->model_tool_image->resize($product_image->row['image'], 98, 98);
            } else {
                $image = $this->model_tool_image->resize('no_image.png', 98, 98);
            }
