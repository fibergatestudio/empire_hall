 ...
 use Helper;
    ...
    for( $i = 0; $i < count($data['options']); $i++) {
        ...
        $name = mb_strtoupper(mb_substr($name, 0, 1, "UTF-8"), "UTF-8").
            mb_substr($name, 1, mb_strlen($name), "UTF-8");
        $data['options'][$i]['name'] = $name;
    }
    //custom code end
...
$data['filtr']=(isset($this->request->get['filter_ocfilter'])?$this->request->get['filter_ocfilter']:false);

        foreach($data['options'] as $o => $option){
            foreach($option['values'] as $v => $value){
                $data['options'][$o]['values'][$v]['href'] = $this->ocfilter->link($value['params']);
            }
        }
