...
class ControllerExtensionTltBlogTltTag extends Controller {
	public function index() { 
		...
		// у нас есть id категории блога - значит выводим нужную категорию
		...
		/* У нас не выбрана категория блога и мы зашли на стартовую страницу всего блога */
		} else {
			...
			$tltblog_tags = $this->model_extension_tltblog_tltblog->getTltTags();
			foreach ($tltblog_tags as $tltblog_tag) {
				$data['tags'][] = array(
					'title' => $tltblog_tag['title'],
					'href'  => $this->url->link('extension/tltblog/tlttag', 'tlttag_id=' . $tltblog_tag['tlttag_id'])
				);
			}
			...
			$this->response->setOutput($this->load->view('extension/tltblog/tltmain', $data));
		}
	}
}