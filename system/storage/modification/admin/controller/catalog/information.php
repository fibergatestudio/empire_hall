...
class ControllerCatalogInformation extends Controller {
	...
	public function add() {
		...
		if (($this->request->server['REQUEST_METHOD'] == 'POST') && $this->validateForm()) {
			...
			if(isset($this->request->post['kfc_feeld']) && $this->request->post['kfc_feeld']){
				...
				$this->model_extension_kcf->getKcfUpdateFeeld($kcf_type, $this->request->get['information_id'], $this->request->post['kfc_feeld']);
			}
	...
	protected function getForm() {
        ...
        // Triggers
        if (isset($this->request->post['information_triggers'])) {
            $data['information_triggers'] = $this->request->post['information_triggers'];
        } elseif (isset($this->request->get['information_id'])) {
            $data['information_triggers'] = $this->model_catalog_information->getInformationTriggers($this->request->get['information_id']);
        } else {
            $data['information_triggers'] = array();
        }
		...
		$data['information_id'] = isset($this->request->get['information_id']) ? $this->request->get['information_id'] : $this->model_catalog_information->getNewInfoId();
		
		/* получим сам набор полей, привязанный к странице */
		$data['kcf_feelds'] = $this->model_extension_kcf->getKcfFeelds($kcf_type, $data['information_id']);
		$data['img_start'] = HTTP_CATALOG . 'image/';
		
		/* Получим данные для Выбора типа полей */
		$data['kcftypes_now'] = $this->model_extension_kcf->getKcfTypeNow($kcf_type, $data['information_id']);
		/* ------ END KCF ------ END KCF ------ END KCF ------ END KCF ------ END KCF ------ END KCF ------ */
		/* !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! */		
		...
		if (isset($this->request->post['cover_image'])) {
			...
			$cover_image = $this->model_catalog_information->getInformationGallery($this->request->get['information_id']);
		} else {
			$cover_image = array();
		}
		...
		foreach ($cover_image as $images) {
			...
			foreach ($images['title'] as $key => $value) {
