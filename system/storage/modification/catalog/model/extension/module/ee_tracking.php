...
//            $fields['ni'] = 1;
    ...
    public function getProduct($product_id, $language_id) {
        $query = $this->db->query("SELECT DISTINCT p.product_id, p.sku, p.model, pd.name AS name, pd.meta_title AS meta_title, (SELECT md.name FROM " . DB_PREFIX . "manufacturer_description md WHERE md.manufacturer_id = p.manufacturer_id AND md.language_id = '" . (int)$language_id . "') AS manufacturer FROM " . DB_PREFIX . "product p 
    ...
    public function getAdvProduct($product_id, $language_id) {
        $query = $this->db->query("SELECT DISTINCT p.*, pd.*, (SELECT md.name FROM " . DB_PREFIX . "manufacturer_description md WHERE md.manufacturer_id = p.manufacturer_id AND md.language_id = '" . (int)$language_id . "') AS manufacturer, 
