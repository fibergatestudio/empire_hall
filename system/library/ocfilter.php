...
final class OCFilter {
  ...
  public function getOptionsGet() {
    ...
    $page_info = $this->getPageInfo();
        ...
        $filters = explode(';', $this->request->get['filter_ocfilter']); //custom

    if ($page_info) {
		  $meta_title = $page_info['meta_title'];
    } else if(count($filters) == 1 && strpos($filters[0], 'm') !== false){
        return $meta_title;
    ...
    } else if(strpos($filters[0], 'm') !== false){
            ...
            if ($this->config->get('module_ocfilter_stock_status_method') == 'stock_status_id') {
                ...
                $options['stock'] = array(
                    ...
                    'values'      => array(
                        array(
                            'value_id'    => 'in',
                            'name'        => $this->language->get('text_only_in_stock')
                        )
                    )
                );
    ...
    $index = 0;
      ...
      if ($option['type'] != 'slide' && $option['type'] != 'slide_dual') {
        ...
        if (!$values || (count($values) < 2 && $option['option_id'] != 'm' && $option['option_id'] != '10089' && $option['option_id'] != 's')) { // custom
        	continue;
        }
      } else {
